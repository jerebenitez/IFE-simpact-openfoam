 ...
 !   READ control DATA for element number 24 (TL BST++ for thermo-mechanical problems)
 ...
 TYPE (ele24_set), POINTER, SAVE  :: elset, anter


 sname = elsnam
 IF (TRIM(task) == 'INPUT ') THEN
   ! check if list of sets and set exists and initializes
   CALL srch_ele24 (head, anter, elset, sname, oldset)
   ...
   ELSE                !set ELSNAM does not exist
     CALL new_ele24(elset)
     CALL listen('INPD24')  !read a line
     ...
     CALL ini_ele24e (elset%head, elset%tail)
   END IF
   !  read new data or add to previous data
   CALL elmd24(nelem, elset%head, elset%tail, iwrit, elset%shear)
   elset%plstr = 0     ! do not compute plastic strains
   IF (.NOT.oldset) CALL rdreqs ( 1 ,nreqs, elset%ngrqs, iwrit )

   CALL comm24(0, nelem,  nreqs, narch, sname, elset, logst)
   ! add to the list of sets
   IF (.NOT.oldset) THEN
     CALL add_ele24 (elset, head, tail)
 ...
 ELSE IF (TRIM(task) == 'RESTAR') THEN
   ...
   CALL rest24 (elset%nelem,  elset%nreqs, elset%head,  elset%tail, elset%shear, &
 ...
 ELSE IF (TRIM(task) == 'IMPORT') THEN
   ! check if list of sets and set exists and initializes
   IF( overw )THEN
     CALL srch_ele24 (head, anter, elset, elsnam, oldset)
     IF (oldset) THEN    !if set exists
       CALL comm24 (1, nelem,  nreqs, narch, sname, elset, logst)
       !elset%lside = .FALSE.   !initializes flag to compute LSIDE
     ELSE
   ...
   END IF
   ! restore list of elements
   CALL impo24 ( nelem,elset%head, elset%tail)
 ...
 ELSE
   CALL runend('INPD24: NON-EXISTENT TASK .        ')
 END IF
